...
const startUpload = qiniuConfig => {
  return new Promise(resolve => {
    ...
    const uploadFile = (uptoken, key, localFile) => {
      ...
      formUploader.putFile(uptoken, key, localFile, putExtra, function (
        err,
        respBody,
        respInfo
      ) {
        if (err) {
          ...
          failedObj.uploadFiles[key] = new moment().format(
            'YYYY-MM-DD HH:mm:ss'
          )
    ...
    const dealFailedFiles = () => {
      const failObj = JSON.parse(
        fs.readFileSync(failedUploadLog, fileEncodeType)
      )
    ...
    const dealFileQN = () => {
      ...
      if (qndataKeysLength !== 0) {
        ...
        bucketManager.batch(deleteOperations, function (
          err,
          respBody,
          respInfo
        ) {
          ...
          if (err) {
            ...
            if (parseInt(respInfo.statusCode / 100) == 2) {
              ...
              if (allFileIsSuccess) {
                console.log(
                  'all extra file is deleted form qiniuCloud successful'
                )
              } else {
    ...
    readFilesFormDir(originPath).then(paths => {
      if (paths.includes(`${originPath}/${filterFile}`)) {
        const filterFileIndex = paths.indexOf(`${originPath}/${filterFile}`)
        paths.splice(filterFileIndex, 1)
      }
